...
const LOCALIZED_COPY = normalizeLocalizedTree({
  bn: {
...
export default function UpdateAvailableBanner({ lang = "en" }) {
  const [nextVersion, setNextVersion] = useState("");
  const [refreshing, setRefreshing] = useState(false);
  const bannerCopy = LOCALIZED_COPY[lang] || LOCALIZED_COPY.en;
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      // Try to navigate to bypass cache
      window.location.assign(refreshUrl);
      // Force reload if navigation gets intercepted
      setTimeout(() => {
  ...
  return (
    <div className="fixed inset-x-4 bottom-4 z-[120] mx-auto max-w-2xl rounded-[1.5rem] border border-primary/20 bg-background/96 p-4 shadow-[0_24px_64px_-28px_rgba(15,23,42,0.45)] backdrop-blur-xl">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="space-y-1">
          <p className="text-sm font-semibold text-foreground">{bannerCopy.title}</p>
          <p className="text-sm text-muted-foreground">{bannerCopy.body}</p>
        </div>
        <Button onClick={handleRefresh} className="shrink-0">
          <RefreshCw className="h-4 w-4" />
          {refreshing ? bannerCopy.working : bannerCopy.action}
        </Button>
      </div>
    </div>
  );
