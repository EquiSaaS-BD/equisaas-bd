...
const applyLedgerToBoard = ({
  ...
  const participantLastAwardedAt = participant.lastAwardedAt ? normalizeDate(participant.lastAwardedAt) : null;
  if (!participantLastAwardedAt || awardedAt >= participantLastAwardedAt) {
    participant.lastAwardedAt = awardedAt.toISOString();
  }

  const boardLastAwardedAt = board.lastAwardedAt ? normalizeDate(board.lastAwardedAt) : null;
  if (!boardLastAwardedAt || awardedAt >= boardLastAwardedAt) {
