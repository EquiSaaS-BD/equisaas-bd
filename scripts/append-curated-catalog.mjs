...
try {
  existingServices = JSON.parse(fs.readFileSync(targetPath, 'utf-8'));
} catch (e) {
  console.error("Error reading existing catalog:", e);
  process.exit(1);
}
...
curatedServices.forEach((service) => {
  let newId;
  do {
    newId = `SRV-${String(nextIdNum++).padStart(4, '0')}`;
  } while (existingIds.has(newId));
  service.id = newId;
});

// Append strictly to the beginning so they show up first
const combinedServices = [...curatedServices, ...existingServices];

try {
  fs.writeFileSync(targetPath, JSON.stringify(combinedServices, null, 2));
  console.log(`Successfully appended 20 curated services to catalog. Total services: ${combinedServices.length}`);
} catch (err) {
